...
export default function BrowsePage() {
  ...
  const nextReadsBooks = useMemo(
    () =>
      userBooks
        .filter((ub) => ub.status === "plan-to-read" && ub.book)
        .map((ub) => ub.book)
        .filter((book): book is BookSearchResult => Boolean(book?.googleBooksId && book?.title)),
    [userBooks]
  );
  ...
  const recentlyAddedBooks = useMemo(
    () =>
      [...userBooks]
        .filter((ub) => Boolean(ub.book?.googleBooksId && ub.book?.title))
        .sort((a, b) => new Date(b.addedAt).getTime() - new Date(a.addedAt).getTime())
        .slice(0, 12)
        .map((ub) => ub.book as BookSearchResult),
    [userBooks]
  );

  const { books: newForYouBooks, isAi: hasAiRecommendations } = useMemo(() => {
    const aiBooks = recommendations
      .filter((rec): rec is BookRecommendation => Boolean(rec?.googleBooksId && rec?.title))
      .slice(0, 12);
    if (aiBooks.length > 0) {
      ...
      {searchQuery.length > 2 ? (
        ...
        <div className="space-y-8">
          {browseCategories.map((category) => {
            ...
            const categoryBooks = (categoryQuery?.data ?? []).filter(
