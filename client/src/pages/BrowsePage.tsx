...
export default function BrowsePage() {
  ...
  const categoryConfigs = useMemo<CategoryConfig[]>(() => {
    return enabledCategories.map((category, index) => {
      ...
      return {
        ...
        formatSlug: category.formatSlugs?.[0] ?? undefined,
        audienceSlug: category.audienceSlugs?.[0] ?? undefined,
        domainSlug: category.domainSlugs?.[0] ?? undefined,
        supergenreSlug: category.supergenreSlugs?.[0] ?? undefined,
        emptyMessage,
  ...
  const openEdit = async (config: CategoryConfig) => {
    ...
    const categoryPreference = {
      ...
      formatSlugs: storedCategory?.formatSlugs ?? [],
      formatNames: storedCategory?.formatNames ?? [],
      audienceSlugs: storedCategory?.audienceSlugs ?? [],
      audienceNames: storedCategory?.audienceNames ?? [],
      domainSlugs: storedCategory?.domainSlugs ?? [],
      domainNames: storedCategory?.domainNames ?? [],
      supergenreSlugs: storedCategory?.supergenreSlugs ?? [],
      supergenreNames: storedCategory?.supergenreNames ?? [],
    };
    ...
    const updated = list.map((c) => (
      c.slug === editSlug
        ? {
            ...
            formatSlugs: categoryUpdates.formatSlugs ?? [],
            formatNames: categoryUpdates.formatNames ?? [],
            audienceSlugs: categoryUpdates.audienceSlugs ?? [],
            audienceNames: categoryUpdates.audienceNames ?? [],
            domainSlugs: categoryUpdates.domainSlugs ?? [],
            domainNames: categoryUpdates.domainNames ?? [],
            supergenreSlugs: categoryUpdates.supergenreSlugs ?? [],
            supergenreNames: categoryUpdates.supergenreNames ?? [],
          }
        : c
    ));
