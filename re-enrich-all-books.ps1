# Re-run improved enrichment tasks for all 20 existing books
# Tasks: 4 (domain/supergenres), 5 (genres/subgenres), 6 (cross-tags), 7 (format/audience), 8 (SQL)
...
Write-Host "Re-running Enrichment Tasks for All Books" -ForegroundColor Cyan
Write-Host "===========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Total books: $($bookIds.Count)" -ForegroundColor Yellow
Write-Host "Tasks to run: 4 (domain/supergenres), 5 (genres/subgenres), 6 (cross-tags), 7 (format/audience), 8 (SQL)" -ForegroundColor Yellow
Write-Host ""
...
foreach ($bookId in $bookIds) {
    $bookIndex++
    Write-Host "===========================================" -ForegroundColor Cyan
    Write-Host "[$bookIndex/$($bookIds.Count)] Processing book: $bookId" -ForegroundColor Cyan
    Write-Host "===========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  📍 Task 4: Domain + Supergenres..." -ForegroundColor White
    ...
    Write-Host "  📍 Task 5: Genres + Subgenres..." -ForegroundColor White
    ...
    # Task 6: Cross-Tags (improved with pattern matching)
    Write-Host "  📍 Task 6: Cross-Tags (with improved patterns)..." -ForegroundColor White
    ...
    # Task 7: Format + Audience (improved format detection)
    Write-Host "  📍 Task 7: Format + Audience..." -ForegroundColor White
    ...
    # Task 8: Generate SQL
    Write-Host "  📍 Task 8: Generate SQL..." -ForegroundColor White
    node enrichment-tasks/task-08-generate-sql.js $bookId
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  ❌ ERROR in Task 8" -ForegroundColor Red
        $taskFailed = $true
    }
    
    if ($taskFailed) {
        Write-Host "  ⚠️  Some tasks failed for this book" -ForegroundColor Yellow
        $failCount++
    } else {
...
Write-Host ""
Write-Host "✅ Re-enrichment complete!" -ForegroundColor Green
Write-Host ""
